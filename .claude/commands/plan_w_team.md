...
TEAM_MEMBERS: `.claude/agents/team/*.md` (if available, otherwise use `general-purpose`)
...
4. Define Team Members - Use `ORCHESTRATION_PROMPT` (if provided) to guide team composition. If `.claude/agents/team/*.md` files exist, use them; otherwise default all team members to `general-purpose`. Document in plan.
...
<continue with additional tasks as needed. Use agent types from .claude/agents/team/*.md if available, otherwise use general-purpose>
