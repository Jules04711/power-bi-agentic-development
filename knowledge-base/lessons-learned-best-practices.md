...
- **AST-parse `.ps1` to syntax-check without executing**: `[System.Management.Automation.Language.Parser]::ParseFile($path, [ref]$null, [ref]$errs)` — a non-empty `$errs` array means syntax errors (each has `.Message` and `.Extent.StartLineNumber`). Ideal for bulk-validating generated automation scripts.
- **PowerShell 5.1 accepts `if` as a parenthesized expression argument**: `Add-Result 'Check' (if ($c -gt 0) { 'FAIL' } else { 'PASS' }) 'detail'` parses and runs — useful for inline status mapping in validators without a temp variable.
